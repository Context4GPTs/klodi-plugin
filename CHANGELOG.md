...
## [0.3.12] — 2026-07-06 — the localhost plaintext bypass and the dead `ws://localhost` WebSocket transport are removed: `tls://` is the sole rule

**All six adapters move to `0.3.12` together (lockstep version line).** This release removes the last of the pre-`tls://` transport scaffolding. `0.3.11` narrowed the guard to `tls://` off-localhost but deliberately KEPT the localhost plaintext bypass and the `ws://localhost` WebSocket transport, because every integration harness still dialed `ws://localhost:8080`. With the harnesses moved to a dev-CA `tls://localhost` loopback (in klodi-stage, now merged), that path is dead scaffolding — so it is deleted. The change lands in the shared `nats-client-{py,ts,rs}` clients that OpenClaw, `klodi-hermes`, `klodi-nanobot`, and the three Rust adapters (`klodi-moltis`, `klodi-ironclaw`, `klodi-zeroclaw`) build against — so no adapter is a republish-only bump. Cert/hostname verification is untouched (ADR-0022 invariant: verify-ON, private-CA-only, no disable toggle). Contract addendum lands on [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md).

### Removed

- **The localhost plaintext bypass is gone — the transport guard's sole rule is now `scheme == tls://`.** The shared guard `assert_tls_or_localhost` / `assertTlsOrLocalhost` is renamed `assert_tls` / `assertTls` (once the localhost carve-out is deleted, the `_or_localhost` suffix is a lie; no re-export of the old name per the no-backwards-compat rule), and the `is_localhost` / `isLocalhost` host helper is deleted entirely. `ws://` / `wss://` / `nats://` are now rejected on **every** host, localhost included — there is no plaintext transport anywhere. `tls://localhost` (dev CA) is accepted *because it is `tls://`*, not via a host carve-out. The rejection message names `tls://` as the sole accepted transport and no longer mentions `ws://` / `wss://` / localhost. The four adapter persist paths inherit this through the same shared guard (no adapter-local scheme check).
- **The `ws://localhost` WebSocket transport machinery is deleted.** Python drops the `_ws_transport_patch` module (the nats-py `WebSocketTransport.readline` monkey-patch) and its connect-time call; TypeScript drops `usesWebSocketTransport` + `connectWebSocket` (the `ws`-package `wsFactory`), the `wsconnect` import, and the `ws` / `@types/ws` dependencies, routing every connect through the raw-TCP `tls://` transport; Rust collapses the dead non-`tls://` else-branch in `client.rs::connect`. `connect()` now routes every accepted URL to the single raw-TCP+TLS transport. Git history is the recovery path if a WS edge ever returns.

## [0.3.11] — 2026-07-05 — the NATS transport guard collapses to `tls://`-only: `wss://` is rejected off-localhost

**All six adapters move to `0.3.11` together (lockstep version line).** This release completes the client half of the `tls://`-only cutover. The shared connect-time + persist-time transport guard — widened to a two-scheme `{wss://, tls://}` allow-list in `0.3.9` (#41) — now narrows to `tls://` as the *sole* accepted non-localhost transport across the `nats-client-{py,ts,rs}` clients that OpenClaw, `klodi-hermes`, `klodi-nanobot`, and the three Rust adapters (`klodi-moltis`, `klodi-ironclaw`, `klodi-zeroclaw`) build against — so no adapter is a republish-only bump. The localhost plaintext bypass, the `ws://localhost` WebSocket dispatch branch, and all cert/CA-verification code are unchanged. Contract addendum lands on [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md).

### Changed

- **The NATS transport guard now accepts only `tls://` off localhost — `wss://` is rejected (all six adapters).** The shared guard `assert_encrypted_or_localhost` / `assertEncryptedOrLocalhost` is renamed `assert_tls_or_localhost` / `assertTlsOrLocalhost` (once `wss://` — an encrypted scheme — is rejected off-localhost, "encrypted" misnames the control; no re-export of the old name per the no-backwards-compat rule). A non-localhost `nats_url` must now use `tls://`; `wss://` / `ws://` / `nats://` are accepted only when the host resolves to localhost, so dev and the three integration harnesses (all on `ws://localhost`) keep working. Both failure surfaces are loud and terminal: a stale persisted `wss://` url is rejected synchronously at `connect()` **before** any transport dispatch (no `wsconnect` attempt, no hang), and a `/register` response carrying a `wss://` `nats_url` fails closed at persist through the same shared guard (the four adapter persist paths carry no adapter-local scheme check). The rejection message names `tls://` as the sole required scheme, lists `wss://` among the rejected ones, and frames re-register as the normal migration remedy. Cert/hostname verification stays ON, private-CA-only, with no disable toggle. [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md) addendum.
- **`KLODI_DEFAULT_NATS_URL` fallback flips from the `wss://` WS edge to the raw-TLS L4 proxy.** The codegen catalog constant (`tool-catalog/src/index.ts`, fanned out to the py/rs mirrors) moves from `wss://klodi-net.4gpts.com` to `tls://hayabusa.proxy.rlwy.net:32770` (the Railway L4 TCP proxy in front of NATS, provisioned 2026-07-04). It stays a pre-registration fallback only — `nats_url` remains server-authoritative. The `_ws_transport_patch` shim is retained (inert on `tls://`, still load-bearing on `ws://localhost`).

## [0.3.10] — 2026-07-04 — the private-CA `tls://` trust chain goes end-to-end: register-response CA auto-trust, loud-fail on a bad CA, single-resolver attribution

**All six adapters move to `0.3.10` together (lockstep version line).** This release completes the private-CA `tls://` work `0.3.9` scaffolded (#41): the trusted CA is no longer an empty placeholder constant but a real, server-delivered, auto-trusted chain that also fails loud when it can't be trusted. All three changes land in the shared `nats-client-{py,ts,rs}` clients that OpenClaw, `klodi-hermes`, `klodi-nanobot`, and the three Rust adapters (`klodi-moltis`, `klodi-ironclaw`, `klodi-zeroclaw`) build against — so no adapter is a republish-only bump. A fourth change catalogs two wake-relay tools and closes an unenforced tool-symmetry gate. Contracts recorded across the [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md) addenda and the [ADR-0014](./docs/decisions/0014-tool-symmetry-axes.md) correction.

### Added

- **Register-response CA auto-trust (all six adapters, #49).** The marketplace `/register` completed-session response's optional `nats_ca` field is now persisted to `${KLODI_HOME}/nats-ca.pem` (mode `0644`, atomic write, last-write-wins rotation) and trusted at `tls://` connect — a host onboards with just "register → done," no manual `KLODI_NATS_CA_FILE`. It slots in as a **new level-2 source** in the now four-level resolution order — `KLODI_NATS_CA_FILE` → persisted register CA → bundled `KLODI_NATS_CA_PEM` → system store — ranked above the bundle (server-authoritative, endpoint-matched to the served `nats_url`, rotatable without a client release) but below the operator override (strict short-circuit: a set `KLODI_NATS_CA_FILE` is never silently overridden). `nats_ca` is OPTIONAL — a CA problem never fails registration, and omission on a later re-register never revokes the persisted file. No net trust-boundary widening: the CA rides the *same* already-trusted HTTPS channel as `nats_url`/`nats_creds`, and it stays verify-ON, private-CA-only, fail-closed. The `verification_never_disabled` ratchets (py/ts/rs) were extended to the new surface. Forward-compatible and inert until the marketplace emits the field. [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md) addendum.

### Fixed

- **A bad served CA now fails loud, terminal, and prompt instead of hanging forever (all six adapters, #50).** A malformed or untrusted served CA used to make the initial `tls://` connect retry indefinitely — the shared client's unbounded reconnect policy (`retry_on_initial_connect` rs, `max_reconnect_attempts=-1` py) is correct for a transient flap but catastrophic for a *deterministic* verify failure that never succeeds — so hermes sat pinned at `bridge_register_starting`, indistinguishable from a network blip. The initial connect is now **classified**: a deterministic CA/TLS-verify failure raises a structured error immediately (`CaTrustError` py/ts, the new `KlodiError::CaTrust { ca_source, message }` variant rs) that names the CA **source** so the operator knows the remedy (re-register, or set `KLODI_NATS_CA_FILE`); transient failures (refused / DNS / reset / mid-handshake drop) still retry, so first-connect resilience does not regress. Verification is still never disabled — this only changes *how* the rejection surfaces. (klodi `tls://` is STARTTLS, so the fix intercepts the client's own post-`INFO` TLS upgrade error rather than probing pre-connect.) [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md) addendum.
- **Two wake-relay tools are now catalogued, and the symmetry gate that missed them is enforced (#48).** `klodi_message_user` and `klodi_pending_decisions` were registered by hermes yet absent from the catalog's `LOCAL_TOOLS`. The first-axis gate `check-adapter-tools.sh` *detected* the drift but nothing asserted its non-zero exit, and it was chronically red from deny-list false-positives (`*_failed` log events, internal package/attr names) so the real signal was buried. Both tools are now in the catalog (`host_shapes: ["in_agent"]`, schemas mirrored byte-for-byte from the hermes registrations — `klodi_pending_decisions.result` is the registry's first top-level array), the deny-list is hardened so a clean tree is green, and a new enforced wrapper test regenerates codegen, shells the gate, and asserts no `unknown: <wake-tool>` with a drop-a-tool regression guard. [ADR-0014](./docs/decisions/0014-tool-symmetry-axes.md) correction.

### Changed

- **CA-source precedence + loud-fail attribution consolidated into the single resolver (internal, #51).** The four-level resolution order was encoded *twice* per language — once in the connect-time resolver, once in a parallel `describe_ca_source` / `describeCaSource` helper used only to name the source in the loud-fail error — six drift sites where a precedence change not mirrored in lockstep would misattribute the served CA and misdirect the operator's remedy. Each resolver (`build_tls_context` py → `(ctx, source)`, `resolveTlsCa` ts, `resolve_ca_file` rs → `ResolvedCa`) now returns the CA material **and** the formatted source label together; the client interpolates that label into the loud-fail error verbatim, and the parallel helper is deleted (no shim, no re-export). Pinned by a per-language `ca_source_parity` test asserting resolver-selected source == attributed source at every level. No behaviour change — a potential-drift refactor that closes a P3 nit deferred by #50. [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md) addendum.

## [0.3.9] — 2026-07-04 — secure `tls://` NATS transport with private-CA trust; the hermes wake relay reliably reaches the operator

**All six adapters move to `0.3.9` together (lockstep version line).** Unlike the last few releases, every adapter carries a real functional change this time: the new secure `tls://` NATS transport (#41) lands in the shared `nats-client-{py,ts,rs}` clients and the register-persist guard that OpenClaw, `klodi-hermes`, `klodi-nanobot`, and the three Rust adapters (`klodi-moltis`, `klodi-ironclaw`, `klodi-zeroclaw`) all build against — so this is not a republish-only bump for any of them. Hermes additionally ships two wake-relay bugfixes (#45, #46). The three contracts are recorded in [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md) and [ADR-0023](./docs/decisions/0023-relay-wake-terminal-disposition-contract.md).

### Added

- **`tls://` NATS transport with private-CA trust (all six adapters).** The shared transport guard now accepts raw NATS-over-TLS (`tls://`, the L4 TCP-proxy path) alongside `wss://`, renamed to `assert_encrypted_or_localhost` / `assertEncryptedOrLocalhost` across the py/ts/rs clients; the previously divergent register-persist guards in `klodi-hermes`, `klodi-nanobot`, OpenClaw, and `klodi-rust-host` (the shared crate the three Rust adapters build on) now all point at that single guard, so connect-time and persist-time transport policy can no longer drift apart. Each language gains a `tls://` connect path with **certificate verification kept on everywhere** — Python via an `ssl.SSLContext` from `build_tls_context`, Rust via `add_root_certificates`, TypeScript via `@nats-io/transport-node` scheme-branched in `doConnect`. The trusted CA is selected by a `KLODI_NATS_CA_FILE` PEM-path override (falling back to the bundled `KLODI_NATS_CA_PEM` tool-catalog constant, an empty placeholder for now) — the override chooses *which* CA to trust, never *whether* to verify. The `_ws_transport_patch` shim is retained but inert on `tls://` (still used for `ws://` dev). Contract: [ADR-0022](./docs/decisions/0022-tls-nats-transport-private-ca-trust.md).

### Fixed

- **First wake after idle no longer no-ops — the wake pump arms only in the bridge (hermes).** `start_wake_pump()` is now gated on a positive, non-inherited context capability marker (`BridgeCtx.klodi_wake_pump_host` / `WAKE_PUMP_HOST_ATTR`), so the durable wake consumers are subscribed in exactly one process — the `klodi-hermes-bridge` daemon. Tools and skills still register in every plugin loader; only the bridge subscribes. This removes the multi-subscriber split-brain in which a non-bridge subscriber no-op'd the first-after-idle wake while the consumer ACKed the drop, leaving the marketplace event silently lost. The capability is a context attribute, not an env var — `inject_message` merges `{**os.environ}` into its children, so an env flag would leak into wake turns and fail *open* (the [ADR-0015](./docs/decisions/0015-gateway-runtime-load-vs-armed-axis.md) trap). ADR-0019 (ACK-on-deterministic-failure) and ADR-0020 (marker-on-exit-0) are untouched.
- **A relay wake reliably delivers its report to the operator (hermes).** A relay wake could run a full agent turn and then end with plain assistant prose — never calling `klodi_message_user` — so the bridge captured and **discarded** that report on exit `0` and the operator was never reached. The delivery *mechanism* (PR #37) already worked; the defect was guidance framing `klodi_message_user` as escalation-only, so a correct relay turn had no delivery path. The fix arms a terminal-disposition contract across four coherent plugin-owned surfaces so the agent cannot re-self-censor: `wake_handlers.py` appends a terse "end in a marketplace action or a `klodi_message_user` delivery, never a bare question" line to every delivery-warranting wake (purely-informational lifecycle wakes get no such line — the over-ping guard); `message.py` broadens the tool description to authorize the needs-the-human case and drops the absolute "not for informational updates" exclusion; and the klodi skill's `SKILL.md`, `tool_inventory.md`, and `negotiation_style` template teach "uncertainty is not informational." No bridge output-path change, no default fallback channel. Contract: [ADR-0023](./docs/decisions/0023-relay-wake-terminal-disposition-contract.md).
...
**All adapters.** Long-running agent sessions need a restart. Agents that pattern-match on the previous flat-string responses break on the first failure response. The agent's recovery prompt should be updated to read the structured envelope per `skill/references/error_envelopes.md` (bundled).
