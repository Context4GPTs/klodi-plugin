...
Each adapter declares its own runtime dependencies. The OpenClaw TypeScript adapter ships workspace deps (`@klodi/tool-catalog`, `@klodi/nats-client`) inside the tarball via `bundleDependencies`; public-registry deps (`@nats-io/*`, `ws`, `tweetnacl`, `@sinclair/typebox`, `gray-matter`) are pinned to exact versions and resolved by the host's `npm install` after extraction. The Python adapters (Hermes, nanobot) ship as wheels with pinned `nats-py` and helper deps. The Rust adapters (Moltis, IronClaw, ZeroClaw) are Cargo binary crates with `async-nats` linked statically.
...
*Why bundleDeps + host `--ignore-scripts` rather than vendoring?* The previous design (see [ADR-0003](docs/decisions/0003-vendored-runtime-dependencies.md), superseded) carried every transitive into `dist/node_modules/` at build time. It maintained two sources of truth (`vendor-deps.mjs` and `package.json#dependencies`) that drifted in practice. The current design ships workspace deps via `bundleDependencies` (preserves reproducibility) and lets the host re-resolve public-registry deps with `npm install --omit=dev --silent --ignore-scripts` (blocks transitive postinstall execution). The plugin pins `openclaw.install.minHostVersion: ">=2026.4.15"` so the install will refuse hosts where the `--ignore-scripts` enforcement has not been verified. See [ADR-0008](docs/decisions/0008-bundled-deps-host-ignore-scripts.md).
