...
- **Pre-existing clippy debt (NOT introduced by this merge, NOT in the 8 conflicts):** `cargo clippy -- -D warnings` flags `uninlined_format_args` in `packages/klodi-rust-host/src/setup_status.rs:410` and `adapters/zeroclaw/src/bin/register.rs:166,170`. Both pre-date the merge (present at round-2 HEAD `0c5bdf5`, untouched by the merge — `git diff 0c5bdf5 -- <file>` is empty) and round-2 Review PASSED with them present, so the effective round-2 clippy gate did not sweep the bins. Left them untouched to keep this diff scoped to the reconciliation; flagging for qa/CQG to decide whether to clean up here or in a follow-up.
