#!/usr/bin/env python3
"""Vendor klodi-nats-client (Rust) into a staged copy of the ZeroClaw adapter.
...
Operations:
    ...
    3.  Rewrite imports:
          ...
          d) Adapter bin sources (build/staged/src/bin/*.rs):
                `klodi_nats_client::` → `klodi_zeroclaw::_natsclient::`
    4.  Patch staged Cargo.toml:
        ...
        — Add transitive deps the vendored module needs and the adapter
          itself does not already declare.
...
def rewrite_adapter_sources() -> tuple[int, int]:
    pattern = re.compile(r"\bklodi_nats_client::")
    lib_count = 0
    bin_count = 0
    ...
    for rs in src.rglob("*.rs"):
        ...
        if rel.parts and rel.parts[0] == "bin":
            ...
            new = pattern.sub("crate::_natsclient::", text)
            if new != text:
                lib_count += 1
        if new != text:
            rs.write_text(new, encoding="utf-8")
...
def inject_pub_mod_natsclient() -> None:
    ...
    insertion = (
        ...
        f"// `{ADAPTER_LIB_NAME}::_natsclient::*`.\n"
        "pub mod _natsclient;\n"
    )
