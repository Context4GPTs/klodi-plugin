#!/usr/bin/env python3
"""Vendor klodi-{nats-client,rust-host,logger} into a staged copy of the ZeroClaw adapter.
...
adapters/zeroclaw/ is never modified, so local dev (`cargo check`,
...
Operations (per module, in order):
    ...
    3.  Write build/staged/src/lib.rs declaring `pub mod _natsclient;`,
        ...
        vendored types via klodi_zeroclaw::_<mod>::*).
    4.  Rewrite imports:
          ...
          c) Adapter bin sources (build/staged/src/bin/*.rs):
                `klodi_<mod>::` → `klodi_zeroclaw::_<mod>::`
...
def write_staged_lib_rs() -> None:
    ...
    Publish-time we need a library so the bins can `use klodi_zeroclaw::_natsclient::*`
    ...
    body = (
        ...
        "//!\n"
        "//! The source tree under adapters/zeroclaw/src/ has no lib.rs —\n"
...
def rewrite_adapter_sources() -> tuple[int, int]:
    ...
    - Files under `src/` (other than `_<mod>/` and `bin/`): `klodi_<crate>::`
      → `crate::_<mod>::`. ZeroClaw's source tree currently has no such
      files, so this branch is dormant for zeroclaw but kept to mirror
