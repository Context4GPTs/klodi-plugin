//! `klodi-zeroclaw-register` — one-shot HTTP registration. After the
//! marketplace persists `nats.creds` + `config.json`, the binary also
//! wires klodi into ZeroClaw's `config.toml` so the agent sees the
//! `klodi-zeroclaw-mcp` server on its next start.
...
#[command(
    ...
    about = "Register klodi for the ZeroClaw adapter and wire its MCP entry into ZeroClaw config.",
...
struct Cli {
    ...
    /// adapter target that doesn't run ZeroClaw (e.g. a remote daemon
...
async fn main() -> Result<()> {
    ...
    if cli.skip_zeroclaw_config {
        println!(
            "Skipping ZeroClaw config.toml update (--skip-zeroclaw-config)."
        );
        return Ok(());
    }
    ...
    println!(
        "Wired klodi into {} — ZeroClaw will spawn klodi-zeroclaw-mcp on next agent start.",
        config_path.display(),
    );
    Ok(())
}
