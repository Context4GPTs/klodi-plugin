//! `klodi-moltis-mcp` — stdio MCP server that exposes klodi's tool
//! catalog and skill bundle to Moltis's agent.
//!
//! Moltis spawns one subprocess per agent session per its
...
#[command(
    ...
    about = "Stdio MCP server exposing klodi tools + skill bundle to Moltis.",
...
async fn main() -> Result<()> {
    ...
    if !creds_path.exists() || !config_path.exists() {
        eprintln!("{}", not_registered_envelope_json("klodi-moltis-register"));
        std::process::exit(1);
    }
