#!/usr/bin/env bash
#
# Stamp Cargo.toml#version into klodi-moltis-side mirrors of the version.
#
# Mirrors klodi-plugin/adapters/openclaw/scripts/stamp-version.mjs.
# Single mirror today: GitHub URLs of shape
#   github.com/Context4GPTs/klodi-plugin/(blob|tree)/v<X.Y.Z>/
# in README.md (and any other doc files added later).
#
# Idempotent: re-running with the same version is a no-op. Refuses to
# stamp non-semver versions. Uses Python for the rewrite because BSD
# sed (macOS) and GNU sed disagree on -i, and the awk fallback gets
# messy with capture-group preservation.
...
# Doc files where versioned URLs may appear. Add as new docs land.
TARGETS=("README.md")
