#!/usr/bin/env python3
"""Vendor klodi-{nats-client,rust-host,logger} into a staged copy of the Moltis adapter.
...
Operations (per module, in order):
    ...
    3.  Write build/staged/src/lib.rs declaring `pub mod _natsclient;`,
        ...
        vendored types via klodi_moltis::_<mod>::*).
    4.  Rewrite imports:
...
def stage_mcp_assets() -> None:
    ...
    # ref, and no test guards it. Do not "consistency-fix" it. See ADR-0017.
...
def strip_mcp_cfg_gates() -> int:
    ...
    it and avoid pulling in `rmcp`. Moltis's published crate, by
...
def write_staged_lib_rs() -> None:
    ...
    Publish-time we need a library so the bins can `use klodi_moltis::_natsclient::*`
    ...
    body = (
        ...
        "//!\n"
        "//! The source tree under adapters/moltis/src/ has no lib.rs —\n"
...
def rewrite_adapter_sources() -> tuple[int, int]:
    ...
    - Files under `src/` (other than `_<mod>/` and `bin/`): `klodi_<crate>::`
      → `crate::_<mod>::`. Moltis's source tree currently has no such
      files, so this branch is dormant for moltis but kept to mirror
