"""Shared installer primitives — used by the Hermes setup CLI.

The nanobot adapter ships its own copy of this module. Any change here
must be mirrored in ``klodi-plugin/adapters/nanobot/nanobot_installer.py`` —
CI enforces bitwise equality via ``klodi-plugin/scripts/check-shared-python.sh``.

All helpers are stdlib-only so downstream adapters inherit the same
zero-extra-dependency posture for the install path. (The runtime
adapter brings in ``klodi-nats-client``, but the installer doesn't —
it runs before pip has a chance to install the runtime deps.)
"""
...
log = logging.getLogger("klodi_hermes.hermes_installer")
