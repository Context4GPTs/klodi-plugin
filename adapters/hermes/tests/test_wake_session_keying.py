...
redesign keys each wake to its CONVERSATION, derived from ``event.kind``:

  * channel.*           -> channel_id      (the buy/sell negotiation thread)
  * offer.* / listing.* / comment.created -> listing_id
  * transaction.*       -> transaction_id
  * search.match        -> search_slug
  * key field absent    -> per-wake ephemeral ``wake-<event_id>`` (NEVER a shared
    session — that would re-introduce the bug)
...
async def test_channel_notification_keys_on_channel_id(kind: str) -> None:
    ...
    <channel_id>`` — the conversation thread, not a shared session."""
    event = _golden(kind)
    assert await _spawned_session(event) == event["channel_id"]


@pytest.mark.asyncio
async def test_channel_message_keys_on_channel_id() -> None:
    """AC-1: ``channel.message`` (its own handler) also keys on channel_id,
    so the whole negotiation thread shares one session."""
    event = _golden("channel.message")
    assert await _spawned_session(event, channel_msg=True) == event["channel_id"]


# ── AC-2: offer.* / listing.* / comment.created -> listing_id ─────────
...
async def test_listing_scoped_kinds_key_on_listing_id(kind: str) -> None:
    ...
    <listing_id>`` — all chatter about one listing shares a session."""
    event = _golden(kind)
    assert await _spawned_session(event) == event["listing_id"]


# ── AC-3: transaction.* -> transaction_id ─────────────────────────────
...
async def test_transaction_golden_kinds_key_on_transaction_id(kind: str) -> None:
    ...
    transaction_id."""
    event = _golden(kind)
    assert await _spawned_session(event) == event["transaction_id"]
...
# ── AC-4: search.match -> search_slug ─────────────────────────────────


@pytest.mark.asyncio
async def test_search_match_keys_on_search_slug() -> None:
    """AC-4: a standing-search match keys on its search_slug, so all hits
    for one saved search share a session."""
    event = _golden("search.match")
    assert await _spawned_session(event) == event["search_slug"]
...
async def test_distinct_keys_yield_distinct_sessions() -> None:
    ...
    sessions — the keying does not collapse them."""
    sessions = [
...
async def test_no_wake_runs_in_the_shared_klodi_wake_session(
    ...
    """AC-5: the literal shared ``klodi-wake`` session is gone from the
    inject path — NO wake of any kind may run in it (it is the exact
    unbounded-growth defect this redesign removes)."""
    session = await _spawned_session(_golden(kind), channel_msg=channel_msg)
    assert session != "klodi-wake", (
        f"{kind} collapsed into the shared klodi-wake session"
    )
...
@pytest.mark.asyncio
async def test_missing_key_field_falls_back_to_ephemeral_wake_event_id() -> None:
    """AC-6: a wake whose key field is absent falls back to a per-wake
    ephemeral ``wake-<event_id>`` session — bounded to that one wake."""
    event = {"kind": "channel.opened", "event_id": "evt-orphan", "buyer_handle": "x"}
    assert await _spawned_session(event) == "wake-evt-orphan"
...
async def test_missing_key_never_collapses_to_a_shared_session(
    ...
    is ephemeral and per-wake — never the shared session and never a
    cross-wake constant."""
    session = await _spawned_session(event)
    assert session == f"wake-{event['event_id']}"
    assert session != "klodi-wake"
...
async def test_terminal_event_issues_drain_at_dispatch_ack_seam(
    ...
    consumer's ``_dispatch_message`` seam, issues the session-drain call
    for its session key — and the message still ACKs (never NAKs). Hermes
    actually reclaiming the session is probe-gated; here we assert the
    CALL, not the reclamation."""
    ...
    assert ctx.drained == [event[key_field]], (
        f"{kind} must drain its {key_field} session at the terminal seam"
    )
...
# ── AC-8: ADR-0019 disposition preserved; alarm carries the session key ─
...
async def test_deterministic_failure_preserves_adr0019_and_carries_session(
    ...
    ctx = BridgeCtx(hermes_bin="/usr/bin/hermes", runner=runner)
    wake_handlers.bind_ctx(ctx)
    event = _golden("offer.proposed")  # session key = listing_id
    ...
    assert event["listing_id"] in msg, "alarm must carry the resolved session key"
